...
const SETTING_KEY = 'mobile_hero_image';

interface MobileHeroImageUploadProps {
  currentUrl: string | null;
}

export default function MobileHeroImageUpload({ currentUrl }: MobileHeroImageUploadProps) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `mobile-hero-image-${Date.now()}.${ext}`;
      ...
      startTransition(async () => {
        try {
          await upsertSiteSetting(SETTING_KEY, url);
          setPreview(url);
          setSuccess('Mobiele hero afbeelding opgeslagen!');
        } catch (err) {
  ...
  async function handleRemove() {
    ...
    try {
      ...
      startTransition(async () => {
        try {
          await deleteSiteSetting(SETTING_KEY);
          setPreview(null);
    ...
    <div className="space-y-4">
      ...
      {preview ? (
        <div className="relative">
          <div className="relative w-full rounded-xl overflow-hidden border border-white/10 bg-white/5" style={{ aspectRatio: '2 / 1' }}>
            <Image src={preview} alt="Mobiele hero afbeelding" fill className="object-cover object-top" />
          </div>
          <button
            type="button"
        ...
        <div
          ...
          className="w-full rounded-xl border-2 border-dashed border-white/15
      ...
      {/* Upload button (als er al een afbeelding is) */}
      {preview && (
        <button
          type="button"
          onClick={() => inputRef.current?.click()}
